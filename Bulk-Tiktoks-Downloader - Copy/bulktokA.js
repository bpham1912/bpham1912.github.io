...
async function main() {
  ...
  var videoDes = await page.evaluate(() =>Array.from(document.querySelectorAll('div.tiktok-1qb12g8-DivThreeColumnContainer.eegew6e2 > div > div > div > a')).map((items) => items.innerText))
    ...
    for (var i=videoDes.length; i--;) {
      videoDes[i] = videoDes[i].slice(0, 20) + '#shorts' + i}; //append #shorts for each video title
      fs.appendFile(username+'.txt', videoDes + '', function (err) {
        if (err) throw err;
    ...
    async function getvideourl(){
      ...
      const API_URL = `https://www.tikwm.com/video/media/hdplay/7171785897724153114.mp4`;
    ...
    const request = https.get(content, function(response) {
        if (response.statusCode === 200) {
            var file = fs.createWriteStream(path + current_name +'.mp4');
            response.pipe(file);
            console.log(file.path + ' Saved!')

            fs.appendFile(username+'.txt',file.path + "\r\n" , function (err) {
            if (err) throw err;
            console.log('Done');
            });
        }
